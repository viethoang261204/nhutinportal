...
function ensureActivityLogSchema(PDO $pdo): void
{
    try {
        ...
        $pdo->exec("CREATE INDEX IF NOT EXISTS idx_activity_entity ON activity_logs (entity_type, entity_id)");
    } catch (Throwable $e) {}
    try {
        $colStmt = $pdo->query(
            "SELECT column_name
        ...
        if (!in_array('user_name', $cols, true)) {
            $pdo->exec("ALTER TABLE activity_logs ADD COLUMN user_name VARCHAR(100) NULL");
        }
        if (!in_array('user_role', $cols, true)) {
            $pdo->exec("ALTER TABLE activity_logs ADD COLUMN user_role VARCHAR(20) NULL DEFAULT 'staff'");
        }
